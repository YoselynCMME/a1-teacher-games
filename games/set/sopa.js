...
{if(botonesOcultos==1)$("#btnMostrar").css("visibility","hidden");};respuestasXAPI=new Array();function inicializarVariablesXapi(){respuestasXAPI=new Array();servidor=location.protocol+"//"+window.location.host
registroURL=registro.getElementsByTagName("url")[0].childNodes[0].nodeValue;time=registroURL.split("t=");t=time[1].split("&");t=t[0];if(typeof(emailAutor)==="undefined"){emailAutor="mailto:exportacion@example.com";}
...
if(typeof(grouping)==="undefined"){var d=new Date();var n=d.getTime();grouping="id:"+n+"/"+Math.random();}
...
function inicializarCorregir(){window.onbeforeunload=function(){}
if(userNombre==" "){userNombre="Anonimo";}
if(userEmail==""){userEmail="anonimo@example.com";}}
function inicializarXapi(){var stmt={"actor":{"objectType":"Agent","openid":"id:"+idUser,"name":""+userNombre},"verb":{"id":"http://adlnet.gov/expapi/verbs/initialized"},"object":{"id":mParent,"objectType":"Activity","definition":{"name":{"es":enunciado},"type":"https://www.educaplay.com/es/actividad/sopa.htm","extensions":{"info:limitetiempo":tiempo,"info:pistas":pistas,"info:intentos":numero_intentos,}},},"context":{"contextActivities":{"grouping":[{"id":grouping}]},"instructor":{"name":autor,"openid":"id:"+idAutor,"objectType":"Agent"},}};$.ajax({type:"POST",url:servidor+"/xapi.php?action=guardar&t="+t+"&idActividad="+idActividad,data:stmt,success:function(msg){console.log(msg);},error:function(){}})}
function completarXapi(tipoFinal,puntos,tiempoReg){var result=0;if(tipoFinal=="OK"){result=1;}
var tiempoR=tiempoReg.split(":");var tiemp=parseInt(tiempoR[0]*60)+parseInt(tiempoR[1]);if(tiempo!=0){tiemp=tiempo-tiemp;}
tiemp="PT"+tiemp+"S"
...
function responderEscritaXapi(npistas,correcta,testrespondida,exito){var stmt1={"object":{"objectType":"Activity","id":mParent,"definition":{"name":{"es":correcta},"type":"https://www.educaplay.com/es/actividad/sopa.htm","extensions":{"pistas":npistas,},"interactionType":"fill-in","correctResponsesPattern":[correcta],}},"result":{"response":testrespondida,"success":exito,},}
respuestasXAPI.push(stmt1);};function comenzar()
{inicializarVariablesXapi();$('#contentPreActividad').hide();$('#contentAct').css('top',0);idInterval=setInterval(contador,1000);if(is_touch_device())
...
if(final==1)
{for(i=0;i<pa.length;i++){pistaUtilizada=0;if(controlAciertos[i]==1){for(j=0;j<arrayMitad.length;j++){if(arrayMitad[j]=i){pistaUtilizada=1}}
...
var puntosReg=0;var arrayMitad=new Array()
function actualizaPuntos(num)
{var puntosSumar=Math.floor(100/pa.length);if((puntosMitad==1)&&(palabraMitad==num)){arrayMitad.push(num);puntosSumar=Math.round(puntosSumar/2);puntosMitad=0;}
...
if(final==1)
{for(i=0;i<pa.length;i++){for(i=0;i<pa.length;i++){pistaUtilizada=0;if(controlAciertos[i]==1){for(j=0;j<arrayMitad.length;j++){if(arrayMitad[j]=i){pistaUtilizada=1}}
responderEscritaXapi(pistaUtilizada,pa[i],pa[i],1)}
else{responderEscritaXapi(pistaUtilizada,pa[i],"",0)}}}
